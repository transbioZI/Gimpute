...
* 6-finalResults: Final processed and imputed genotype files are generated in this directory.
...
```{r} 
## Define the directory where you dowload/place the imputation reference files 
...
0. Locate the original PLINK files and other requird files either from our package or your side.
...
* Genotype data after converting to the targeted genome build:../1-genoUpdate/1_11_removedYMtSnp.#
...
## remove unwanted files in this dir
system(paste0("rm  *.log ", metaDataFile))
## change dir 
setwd("..") 
```
...
.prepareLegend2bim(inputFile, outputFile=referenceFile, ncore=25) 
...
## 4. Remove SNPs which have an allele which is not 
## in the imputation reference files for that SNP.
...
system(paste0("cp ./3-checkAlign/", inputPrefix4aligned2impRef, ".* ./4-imputation/"))
...
* The SNPs with different positions: ../3-checkAlign/3_2_snpDiffNamePos.txt
* The dataset before removing SNPs for imputation: ../3-checkAlign/3_1_liftedDataset.#
...
```{r eval=FALSE} 
## imputed dataset
system(paste0("scp ./1-genoUpdate/1_01_metaData.txt ./6-finalResults/metaData.txt " ))
