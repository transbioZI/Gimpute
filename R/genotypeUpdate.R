...
#'
#' @description
#' Remove duplicated sample IDs, which should be defined in the configuration folder in advance. 

#' @param plink an executable PLINK program in either the current working directory or somewhere in the command path.
#' @param dupSampleIDFile a pure text file that stores the duplicated sample IDs, each ID per line. 
#' If it is null, then copy and paste the input PLINK files from the last step as the output files.
...
## Find the shared Ids between plink files and metaData file 
##    a. update the GroupId to the right format according to the metaData file.
## 	  b. Group should be 1 and 2. (1=unaff, 2=aff, 0=miss) instead of case 1 control 0; missing phenotype will be indicated as -9.
##    c. Sex information (1 or M = male, 2 or F = female, 0 = missing); --update-sex expects a file with FIDs and IIDs in the first two columns, the 3rd column is the sex information.
# metaData: Thie file is described beforehand. 
...
replaceGroupIdAndSex <- function(plink, inputPrefix, metaDataFile, outputPrefix){
	...
	system( paste0(plink, " --bfile ", inputPrefix, " --pheno pheno.txt --make-bed --out ", outputPrefixVgroup) )
	system( "rm pheno.txt" )


	## update the sex information according to the metaData file 
	fam[,5] = metadataSubsort[,"sex"]  
	fam[missIDsIndex, 5] = 0  ## keep the IDs of no missing sex info as 0
 	updateSex = fam[,c(1,2,5)]  ##  a pheno file that contains 3 columns (one row per individual)
 	write.table(updateSex, file="updateSex.txt", quote=F, row.names=F, col.names=F, eol="\r\n", sep=" ")  

	## Alternate phenotype files 
	system( paste0(plink, " --bfile ", outputPrefixVgroup, " --update-sex updateSex.txt --make-bed --out ", outputPrefix) )
...
##########################################   
##########################################   
#' Check if all study SNPs are included in the chip annotation file. If some of study SNPs are not included then remove them.
#'
#' @description
...
#'
#' @description
#' Remove duplicated probes or SNPs that have same rs-names but different versions of SNP ID (e.g. SNP-A IDs for Affymetrix chip) found in chip annotation information (For Affymetrix chip and PsychChip). 
 	...
 	if (!is.null(chipAnnoFile)){
		...
		for (i in chrNames) { 
			...
			subData = comb[which(comb[,1]==i), ]  
	  		subDup = subData[duplicated(subData[,"pos"], fromLast=TRUE), ] ## The 1st duplicated ID (or 2nd if there are three replicates) will be removed.
	  		print(dim(subDup))
...
#'
#' @description
#' Update SNP information including SNP name, genomic position, chromosome location and the strand information, with the help of the chip annotation file. 
#' This chip annotation file is defined in the configuration folder. 
...
updatedSnpInfo <- function(plink, inputPrefix, chipAnnoFile, chipType, outputPrefix){
 	...
 	if (!is.null(chipAnnoFile)){
		...
		if (chipType=="affymetrix") { 
				...
				updateSNPbackward = comV2[which(comV2[,"strand"]=="-"), "rsID"]  ## strand 
	 	} else if (chipType=="illumina"){ 
				...
				updateSNPbackward = comV2[which(comV2[,"strand"]=="-"), "V2"]  ## strand 
	 	 } else if (chipType=="PsychChip"){ 
		    	updateSNP2rs = subset(comV2, select=c(V2, V2)) ## no need to change but for consistency with Affymtrix
				updateSNPchr = subset(comV2, select=c(V2, chr))
				updateSNPpos = subset(comV2, select=c(V2, pos)) 
...
#'
#' @description
#' Remove SNPs on the chromosome Y and mitochondria (in this case, chromosome==24 & chromosome==26 are removed). If no such SNPs, the output files remain the same as the input.
...
#' @return  a pure text file that stores the prepared chip annotation information in user-defined format. 
#' @export 
